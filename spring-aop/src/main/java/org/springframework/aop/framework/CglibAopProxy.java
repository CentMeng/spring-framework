...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			//如果这个目标类是 CGLIB 代理生成的，则获取其父类 ， 并且把当前类实现的接口添加到 Advisor 配置信息中
			...
			//创建一个 CGLIB 的 enhancer，它是使用 ASM 字节码技术生成的对象实例。每一个 CGLIB 代理生成的对象实例都是 enhancer 类的子类
			...
			//获取 Advised 配置信息中需要被代理的接口 ， 并且将这些接口类设置到新的 CGLIB 代理实例中
			enhancer.setInterfaces(AopProxyUtils.completeProxiedInterfaces(this.advised));
			enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
			enhancer.setStrategy(new ClassLoaderAwareGeneratorStrategy(classLoader));
			//设置 callback，相当于 JDK 动态代理中的 InvocationHandler，是真正执行拦截处理的回调函数
			...
			//使用生产的 enhancer 实例生成代理类的 class 并且实例化当前 class，返回当前新实例化的对象
